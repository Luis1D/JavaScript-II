...
fullName = [];
let getFullName = runners.forEach(index => fullName.push(index.first_name + " " + index.last_name));
...
let largeShirts = runners.filter(index => index.shirt_size == "L");
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result

let ticketPriceTotal = runners.reduce((total, eachDonation) => total + eachDonation.donation, 0);
...
// Problem 1
// Provide the first & last names of people who donated less $100
let arr = [];
const notEnough = runners.forEach(function(index) {
    if(index.donation < 100) {
        let fullNames = index.first_name + " " + index.last_name;
        arr.push(fullNames);
    }
});
console.log(arr);
...
// Problem 2
// Provide how many small t-shirts are needed
const smallShirts = runners.filter(function(index) {
    if (index.shirt_size == "S") {
        return index;
    }
})
...
// Problem 3
// How many names start with the letter 'M'
let startsWithA = 0;
const letterA = runners.filter(function(index) {
    let firstName = index.first_name.split("");
    if(firstName[0] == "M") {
        startsWithA += 1;
    }
})
