console.log("********** START OF ARRAY-METHODS.JS **********");
...
function mapToArr() {
    fullName = runners.map(getFullName);
...
function getFirstNames(item) {
    allCaps = item.first_name.toUpperCase();
    return allCaps;
}
function addToAllCaps() {
    allCaps = runners.map(getFirstNames);
    console.log(allCaps);
}
addToAllCaps();
...
let largeShirts = [];
let findLargeShirts = runners.filter(function(arr) {
    if (arr.shirt_size == "L") {
        largeShirts = arr;
...
console.log("********** END OF ARRAY-METHODS.JS **********");